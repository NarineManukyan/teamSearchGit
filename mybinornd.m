function r = mybinornd(n,p,rsm,varargin)
...
% [err, sizeOut] = statsizechk(3,n,p,varargin{:});
...
if isscalar(n) && isscalar(p)
    if (0 <= p && p <= 1) && (0 <= n && round(n) == n)
        r = cast(sum(rand(rsm,[sizeOut,n]) < p, ndimsOut+1), outType);
    else
